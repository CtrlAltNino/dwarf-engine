...
namespace Dwarf
{
	EditorModel::EditorModel(std::string_view name, std::filesystem::path projectPath) : m_ProjectName(name), m_ProjectPath(projectPath) {}
	...
	EditorSelection::EditorSelection(Ref<Scene> scene) : m_Scene(scene) {}
	...
	void EditorSelection::AddEntityToSelection(Entity entity)
	{
		...
		while ((cursor != m_SelectedEntities.end()) && ((cursorIndex = GetTreeIndex(*cursor)) < index))
		{
			cursor++;
		}
	...
	void EditorSelection::ClearEntitySelection()
	{
		...
		m_SelectionType = m_SelectedAsset.empty() ? CURRENT_SELECTION_TYPE::NONE : CURRENT_SELECTION_TYPE::ASSET;
	}

	void EditorSelection::ClearAssetSelection()
	{
		m_SelectedAsset.clear();
		m_SelectionType = m_SelectedEntities.empty() ? CURRENT_SELECTION_TYPE::NONE : CURRENT_SELECTION_TYPE::ENTITY;
	}

	void EditorSelection::RemoveEntityFromSelection(Entity entity)
	{
		m_SelectedEntities.erase(std::find(m_SelectedEntities.begin(), m_SelectedEntities.end(), entity));
