...
namespace Dwarf
{
    ...
    std::shared_ptr<AssetReference<ModelAsset>> modelAsset)
  {
    ...
    s_Camera->GetTransform()->position = { 0,
                                           0,
                                           1.3 * s_MaxDistance * s_Distance };
    s_Camera->SetRenderPlaneParameters(
    ...
    std::shared_ptr<AssetReference<ModelAsset>> modelAsset)
  {
    ...
    s_Camera->GetTransform()->position =
      InterpolateVectors(s_Camera->GetTransform()->position,
                         { 0, 0, 1.3 * s_MaxDistance * s_Distance },
                         s_ScrollSpeed);

    s_Framebuffer->Bind();
    Renderer::Get()->RenderModelPreview(
      modelAsset,
      s_Camera,
      { s_Framebuffer->GetSpecification().Width,
        s_Framebuffer->GetSpecification().Height },
      s_ModelRotationQuat);
    ...
    std::shared_ptr<AssetReference<MaterialAsset>> materialAsset)
  {
    ...
    s_Framebuffer->Bind();
    Renderer::Get()->RenderMaterialPreview(
      materialAsset,
      s_Camera,
      { s_Framebuffer->GetSpecification().Width,
        s_Framebuffer->GetSpecification().Height },
  ...
  PreviewRenderer::UpdateRotation(glm::vec2 deltaMousePos)
  {
    s_ModelRotationTarget = {
      std::max(-90.0f,
               std::min(90.0f, s_ModelRotationTarget.x + deltaMousePos.y)),
      fmodf(s_ModelRotationTarget.y + deltaMousePos.x, 360.0f),
      0
    };
    s_ModelRotation = InterpolateVectors(
      s_ModelRotation, s_ModelRotationTarget, s_RotationSpeed);
    s_ModelRotationQuat = glm::rotate(glm::rotate(glm::quat({ 0, 0, 0 }),
                                                  s_ModelRotation.x * DEG_2_RAD,
                                                  { 1, 0, 0 }),
                                      s_ModelRotation.y * DEG_2_RAD,
                                      { 0, 1, 0 });
  }
}