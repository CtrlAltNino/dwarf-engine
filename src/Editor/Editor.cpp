...
namespace Dwarf
{
	...
	void Editor::Init(std::filesystem::path const &projectPath)
	{
		...
		s_Instance->m_Model = CreateRef<EditorModel>(EditorModel(projectSettings["projectName"].get<std::string_view>(), projectPath));
		...
		if (projectSettings.contains("lastOpenedScene"))
		{
			...
			if (lastOpenedSceneAsset)
			{
				...
				s_Instance->m_Model->SetScene(SceneUtilities::LoadScene(lastOpenedSceneAsset->GetAsset()->m_Path));
			}
			else
			{
	...
	bool Editor::Run(std::filesystem::path const &projectPath)
	{
		...
		while (!s_Instance->m_Window->ShouldClose() && !s_Instance->m_Model->GetCloseSignal())
		{
			// ===== Time related stuff
			lastFrameStamp = currentFrameStamp;
			currentFrameStamp = TimeUtilities::GetCurrent();
			s_Instance->m_Model->SetDeltaTime(TimeUtilities::GetDifferenceInSeconds(currentFrameStamp, lastFrameStamp));
