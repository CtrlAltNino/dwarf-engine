...
OpenGLTexture::OpenGLTexture(Ref<TextureParameters> parameters,
  ...
  case TEXTURE_1D:
    ...
    glTextureSubImage1D(m_Id, 0, 0, GetSize().x, textureFormat, textureDataType,
                        data->ImageData.GetData());
    break;
  case TEXTURE_2D:
    glTextureStorage2D(m_Id, 1, textureFormat, GetSize().x, GetSize().y);
    glTextureSubImage2D(m_Id, 0, 0, 0, GetSize().x, GetSize().y, textureFormat,
                        textureDataType, data->ImageData.GetData());
    break;
  case TEXTURE_3D:
    glTextureStorage3D(m_Id, 1, textureFormat, GetSize().x, GetSize().y,
                       GetSize().z);
    glTextureSubImage3D(m_Id, 0, 0, 0, 0, GetSize().x, GetSize().y, GetSize().z,
                        textureFormat, textureDataType,
                        data->ImageData.GetData());
    break;
  case TEXTURE_CUBE_MAP:
    glTextureStorage2D(m_Id, 1, textureFormat, GetSize().x, GetSize().y);
    glTextureSubImage3D(m_Id, 0, 0, 0, 0, GetSize().x, GetSize().y, 6,
                        textureFormat, textureDataType,
                        data->ImageData.GetData());
    break;
  }
...
uintptr_t OpenGLTexture::GetTextureID() { return m_Id; }
} // namespace Dwarf