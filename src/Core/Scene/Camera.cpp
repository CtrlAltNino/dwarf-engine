...
#define DEFAULT_CAMERA_ROTATION glm::vec3(30, 0, 0)
  ...
  glm::vec2
  Camera::GetRenderPlaneParameters() const
  {
  ...
  Camera::GetViewMatrix() const
  {
    glm::mat4 rot = glm::rotate(glm::mat4(1.0f),
                                m_Transform->rotation.x * DEG_2_RAD,
                                glm::vec3(1.0f, 0.0f, 0.0f)) *
                    glm::rotate(glm::mat4(1.0f),
                                m_Transform->rotation.y * DEG_2_RAD,
  ...
  void
  Camera::SetRenderPlaneParameters(glm::vec2 planeParameters)
  {
  ...
  // ========== Camera Functions ==========
  void
  Camera::OnUpdate(double deltaTime) const
  {
    using enum KEYCODE;
    ...
    if (deltaMousePos.length() > 0)
    {
      ...
      m_Transform->rotation.x =
        std::min(std::max(m_Transform->rotation.x, -90.0f), 90.0f);
    ...
    if (glm::length(movementVector) > 0)
    {
      ...
      movementVector *= deltaTime * m_MovementSpeed *
                        (InputManager::GetKey(KEYCODE::LEFT_SHIFT) ? 2 : 1);
