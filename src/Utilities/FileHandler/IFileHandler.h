...
namespace Dwarf
{
  ...
  public:
    ...
    virtual bool
    FileExists(std::filesystem::path const& filePath) = 0;

    /// @brief Reads a file and returns the content.
    /// @param filePath Absolute path to a file.
    /// @return The content of the file.
    virtual std::string
    ReadFile(std::filesystem::path const& filePath) = 0;
    ...
    /// @param path Path to a directory.
    virtual void
    CreateDirectoryAt(std::filesystem::path const& path) = 0;

    /// @brief Opens a path into a file browser.
    /// @param path Path to open.
    virtual void
    OpenPathInFileBrowser(std::filesystem::path const& path) = 0;

    virtual void
    LaunchFile(std::filesystem::path path) = 0;
    ...
    Rename(std::filesystem::path const& oldPath,
           std::filesystem::path const& newPath) = 0;

    virtual void
    Duplicate(std::filesystem::path const& path) = 0;
