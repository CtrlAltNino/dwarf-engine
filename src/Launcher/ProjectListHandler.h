...
namespace Dwarf
{
  ...
  public:
      ...
      if (!fileContent.empty())
      {
        ...
        if (jsonObject.contains("projectInformation"))
        {
          ...
          foundInfo.graphicsApi =
            jsonObject["projectInformation"]["graphicsApi"];
    ...
    /// @brief Adds a project to the project list and saves the new list to the
    /// disk.
    ...
    OpenAddProjectWindow()
    {
      ...
      // launcher window
      //(if that is not the default behaviour, finding out when implementing the
    ...
    ChangeGraphicsApi(int id, GraphicsApi api)
    {
      ...
      std::filesystem::path projectSettingsPath =
        (s_ProjectList[id].path / "projectSettings.dproj").c_str();
      // size_t pos;
